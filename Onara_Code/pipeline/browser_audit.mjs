...
if (args[0] === "--batch") {
  isBatch = true;
  batchConfigPath = args[1];
  if (!batchConfigPath) {
    throw new Error("Usage: node browser_audit.mjs --batch <config.json>");
  }
} else {
  [singleInput, singleOutputDir, singleReportPath] = args;
  if (!singleInput || !singleOutputDir || !singleReportPath) {
    throw new Error("Usage: node browser_audit.mjs <input.html> <output-directory> <report.json> OR --batch <config.json>");
  }
}

const batch = [];
if (isBatch) {
  const content = await readFile(batchConfigPath, "utf8");
  batch.push(...JSON.parse(content));
} else {
  batch.push({
    inputPath: singleInput,
    outputDirectory: singleOutputDir,
...
try {
  ...
  for (const item of batch) {
    await mkdir(item.outputDirectory, { recursive: true });
    const html = await readFile(item.inputPath, "utf8");
    currentHtml = html;
    ...
    for (const viewport of [
      ...
      if (viewport.key !== "reflow") {
        const screenshotPath = resolve(item.outputDirectory, `${viewport.key}.jpg`);
    ...
    await writeFile(item.reportPath, JSON.stringify(report), "utf8");
  }
} finally {
