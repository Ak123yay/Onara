...
- Phase 22 Supabase persistence: upserts the completed generated site into `public.projects` with business facts, style preferences, GitHub path, Cloudflare project name, public Onara URL, generation duration, and live status when `SUPABASE_URL` plus a service-role key are configured.
...
- `/pipeline/status/{job_id}` exposes `current_agent`, `agents_completed`, `completed_agent_ids`, `preview_html`, `component_file_count`, `deployment_file_count`, `deployment_manifest`, `github_commit_status`, `github_commit_sha`, `github_commit_url`, `github_commit_path`, `github_repository`, `cloudflare_deployment_status`, `cloudflare_deployment_url`, `cloudflare_project_name`, `cloudflare_project_url`, `supabase_project_status`, `supabase_project_id`, `supabase_project_error`, `photo_count`, `photo_resolution_status`, `qa_status`, `qa_checks`, `qa_blocking_issues`, `qa_warnings`, `mobile_status`, `mobile_checks`, `mobile_issues`, `mobile_fixes`, `supervisor_decision`, `blackboard_keys`, and progress log entries.
